...
from accessibility import AccessibleEntity, AccessibilityNeed
...
class Line(AccessibleEntity):
    ...
    def is_accessible_for(self, needs: set[AccessibilityNeed]) -> bool:
        if not self.supports_all(needs):
            return False

        return any(vehicle.supports_all(needs) for vehicle in self.all_vehicles())
